...
export default function App() {
    ...
    const getHadiths = useCallback(async () => {
        ...
        try {
            ...
            // if web set to url
            if (Platform.OS === 'web') {
                setHadithURL(randomBook['book_key'],
                    randomChapter['chSerial'],
                    randomHadith['hadithNo'])
            }
    ...
    useEffect(() => {
        ...
        if (Platform.OS === 'web') {
            Linking.getInitialURL().then(url => {
                ...
                if (queryParams.book_key, queryParams.chapterID, queryParams.hadithNo) {
                    getThisHadith(queryParams.book_key, queryParams.chapterID, queryParams.hadithNo);
                } else {
    ...
    return (
        <>
            ...
            />
            <TouchableOpacity style={styles.refreshIconContainer} onPress={() => {
                getHadiths().then((data) => {
                    setHadith(data);
                    setView('hadith')
                })
...
const styles = StyleSheet.create({
    ...
    refreshIcon: {
        width: 50, height: 50, maxWidth: '100%'
    }
})